...
En mode proxy, les requêtes Responses sortantes sont réécrites avec `store: false` et leur champ `metadata` de premier niveau est retiré pour assurer la compatibilité upstream. Cette réécriture est limitée au trafic Codex Responses et n’affecte pas Claude. En mode direct, le wrapper ne réécrit pas le corps ; Codex CLI 0.144.6 envoie lui-même `store: false` aux fournisseurs Responses personnalisés hors Azure. Ces champs de requête ne constituent pas une garantie contractuelle sur la politique de conservation du fournisseur upstream.
