...
test('Claude Desktop profile metadata restores loopback URL, service, and strategies', (t) => {
  ...
  const applied = applyProxyClaudeDesktop({
    serviceValue: 'llm', strategyValue: 'divers', strategyValues: ['divers'],
    routes: modelRoutesForDesktopMapping('llm', ['divers']),
  ...
  assert.deepEqual(credential.metadata.strategies, ['divers']);
...
test('Desktop stored proxy configuration helpers restore and reconcile explicit choices', () => {
  const credential = {
    host: '::1', port: 17000,
    metadata: { schemaVersion: 1, mode: 'proxy', service: 'llm', strategy: null, strategies: ['divers'] },
  };
  const baseOptions = parseOptions([]);
  const stored = storedProxyConfig(credential, baseOptions);
  assert.deepEqual(stored, { serviceValue: 'llm', strategyValue: 'claude', strategyValues: ['divers'], host: '::1', port: 17000 });
  ...
  const requestedOptions = parseOptions(['--service', 'llm', '--strategy', 'divers', '--host', '[::1]', '--port', '18000']);
  const requested = requestedProxyConfig(requestedOptions);
  assert.deepEqual(requested, { serviceValue: 'llm', strategyValue: 'divers', strategyValues: null, host: '::1', port: 18000 });
  const reconciled = mergeExplicitProxyConfig(stored, requestedOptions);
  assert.equal(reconciled.strategyValue, 'divers');
